...
function MainPage() {
  ...
  const HandleSubmit = (event) => {
    ...
    const options = {
      ...
      headers: {
        "X-RapidAPI-Key": "6b559bc4f0msh0633ce92856a6f4p15ab48jsn32629c967b2b",
  ...
  //console.log(error)
  return (
    <div className="bg-background min-h-screen mt-1 border-2 rounded-lg border-red-600">
      <div className=" font-Poppins max-w-full p-2  md:p-4 cursor-pointer ">
        <b className="text-red-600 text-4xl ">Jobs</b> <p className="text-xl inline"> Portal </p>
        <hr class="w-full h-2 border-dotted my-0 bg-blue-500 border-1 rounded-md  dark:bg-gray-700"></hr>
      </div>
      {error != null &&  (
      ...
      <header className="font-Roboto max-w-full 
                         ...
                         bg-[url('https://images.unsplash.com/photo-1451187580459-43490279c0fa?w=500&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Mnx8YmFja2dyb3VuZCUyMGltYWdlJTIwZGFya3xlbnwwfHwwfHx8MA%3D%3D')] 
                         bg-left-bottom 
        ...
        <form
          ...
          className="bg-white flex items-center 
                     ...
                     md:gap-8 sm:gap-4 shadow focus-within:shadow-inner "
        >
          <MdWorkOutline className="flex-none text-sm m-2 
                                      sm:m-4 text-gray-300 sm:text-xl " />
          <input
            type="text"
            value={query}
          ...
          <input
            ...
            className="bg-blue-700 text-white px-4 
                       ...
                       sm:py-3 sm:text-lg cursor-pointer 
                     hover:bg-white hover:text-blue-700 hover:border-4 hover:border-blue-700 hover:font-bold "
          />
        </form>
      </header>
      <main className="flex flex-col w-full md:flex-row">
        <form className="p-4 py-8 font-Roboto text-secondary 
          ...
          Full Time
          <br />
          <label className="font-bold text-blue-400">Location</label>
          <div className="flex items-center bg-white 
                            shadow rounded p-4 my-4 gap-2 w-ful hover:border-green-700 hover:ring hover:ring-green-700 ">
            <BiWorld />
            <input
              type="text"
              className=" group text-sm outline-none grow w-full"  
          ...
          <label>
            <input
              type="radio"
              name="radio-location"
              className="w-4 h-4 mx-3 mb-4 focus:accent-pink-500"
              value="India"
        ...
        <section className="p-4 w-full md:w-2/3 md:p-8 ">
          { demo.data.map((item,i) => <JobCard key={i} data={item}/>) }
          {apiData == null ? (
